      ...
      steps {
          script {
            ...
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
               sh '''set -o pipefail; docker run -i --rm --name="$BUILD_TAG-gitflow-sn" -e GIT_BRANCH="$BRANCH_NAME" -e GIT_NAME="$GIT_NAME" eeacms/gitflow /checkSonarqubemaster.sh | tee checkresult.txt; sed -i '1d' checkresult.txt'''
             }
